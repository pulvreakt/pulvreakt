...
internal class SimpleComponentManager : ComponentManager {
    ...
    override suspend fun start(component: ComponentRef): Either<ComponentManagerError, Deferred<Either<ComponentError, Unit>>> =
        coroutineScope {
            either {
    ...
    override suspend fun stop(component: ComponentRef): Either<ComponentManagerError, Unit> =
        either {
            val candidateComponent = componentContainer.keys.firstOrNull { it.getRef() == component }
            ensureNotNull(candidateComponent) { ComponentNotRegistered(component) }
    ...
    override suspend fun stopAll(): Either<ComponentManagerError, Unit> = componentContainer.keys.map { it.getRef() }.forEach { stop(it) }.right()

    override fun alive(): Set<ComponentRef> =
        componentContainer
