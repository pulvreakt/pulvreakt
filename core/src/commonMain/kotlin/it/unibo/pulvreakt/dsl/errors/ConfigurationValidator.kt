...
internal class ConfigurationValidator<ID : Any>(
    private val systemSpec: ConfiguredDeviceStructure,
    private val deploymentSpec: ConfiguredDevicesRuntimeConfiguration<ID>,
    private val infrastructure: NonEmptySet<Host>,
) {
    fun validate(): Either<Nel<ConfigurationError>, Unit> =
        either {
    ...
    private fun uniqueDevicesNameOrRaise(names: List<String>): Either<Nel<DuplicateDeviceName>, Unit> =
        either {
    ...
    private fun deploymentConfigurationOrRaise(names: List<String>): Either<Nel<NoDeviceFound>, Unit> =
        either {
    ...
    private fun componentsRegisteredOrRaise(): Either<Nel<ComponentNotRegistered>, Unit> =
        either {
            val missingComponents =
                systemSpec.map { (deviceName, graph, _) ->
    ...
    private fun validHostOrRaise(): Either<Nel<UnknownHost>, Unit> =
        either {
            val unknownHosts =
                deploymentSpec
    ...
    private fun validStartupHostOrRaise(): Either<Nel<InvalidStartupHost>, Unit> =
        either {
            val invalidStartupHosts =
                systemSpec.map { (deviceName, _, componentsCapabilities) ->
                    val deviceStartupHosts =
                        deploymentSpec
