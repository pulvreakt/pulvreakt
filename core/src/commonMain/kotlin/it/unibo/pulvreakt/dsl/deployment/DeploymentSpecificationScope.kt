 ...
 */
class DeploymentSpecificationScope(private val systemConfiguration: ConfiguredDeviceStructure) {
    ...
    fun device(name: String, config: DeviceDeploymentSpecificationScope.() -> Unit) {
        val configuration = either {
            ...
            val scope = DeviceDeploymentSpecificationScope(name, deviceConfig).apply(config)
            scope.generate().bind()
        }
