...
class PulverizationScope<ID : Any> {
    ...
    private var deploymentConfigSpec: Either<Nel<ConfigurationError>, ConfiguredDevicesRuntimeConfiguration<ID>>? = null
     ...
     */
    fun extendedLogicDevice(config: ExtendedDeviceScope<ID>.() -> Unit): ReadOnlyProperty<Any?, LogicDeviceType> =
        ReadOnlyProperty { _, property ->
            val deviceScope = ExtendedDeviceScope<ID>(property.name).apply(config)
            systemConfigSpec.add(deviceScope.generate())
            LogicDeviceType(property.name)
        }
    ...
    fun deployment(
        ...
        protocol: Protocol,
        deploymentConfig: DeploymentSpecificationScope<ID>.() -> Unit,
    ) {
        this.protocol = protocol
        this.infrastructure = infrastructure
        val result =
            either {
                val deploymentScope = DeploymentSpecificationScope<ID>().apply(deploymentConfig)
                deploymentScope.generate().bind()
            }
        deploymentConfigSpec = result
    }

    internal fun generate(): Either<Nel<ConfigurationError>, PulvreaktConfiguration<ID>> =
        either {
            zipOrAccumulate(
            ...
            val systemDevicesSpec =
                either { systemConfigSpec.map { it.bind() } }
                    .map { it.toNonEmptySetOrNull()!! } // Safe since first check

            val (systemSpec, deploymentSpec) =
                zipOrAccumulate(
