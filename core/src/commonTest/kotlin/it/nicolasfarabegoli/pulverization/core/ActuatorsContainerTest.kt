...
class ActuatorsContainerTest : FunSpec(
    {
        context("ActuatorsContainer tests") {
            test("The container when queried with an `Actuator` type should give the right instance") {
                ...
                container.get(MyActuator1::class)?.let { it shouldBeSameInstanceAs myActuator1 }
                container.get<MyActuator1>()?.let { it shouldBeSameInstanceAs myActuator1 }
                container.get(MyActuator2::class) shouldNotBe null
                container.get(DummyActuator::class) shouldBe null
                container.get<DummyActuator>() shouldBe null
            }
